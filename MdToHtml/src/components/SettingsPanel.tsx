...
import { X, Folder, Settings, RefreshCw, FileText, Database, Trash2, AlertTriangle, Edit2, RotateCcw, Code, FileJson, ArrowLeft } from 'lucide-react';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen, onClose, settingsType }) => {
  ...
  const notifyPathsUpdated = () => {
    console.log('Settings updated, triggering app-paths-updated event');
  ...
  const handleUpdateSortMethod = async (method: SortMethod) => {
  ...
  const handleToggleShowDivider = async (value: boolean) => {
    ...
    if (success) {
      fetchInfo();
      notifyPathsUpdated(); // Refresh document list and trigger re-render
    } else {
      alert('保存渲染配置失败');
    }
  };
                  ...
                  <div className="bg-bg-page rounded-lg p-4 border border-border-soft space-y-4 pl-4 border-l border-border-soft">
                      ...
                      <div className="pt-4 border-t border-border-soft">
                          ...
                          <div className="flex flex-wrap gap-2">
                              {
                                [
                                  { value: 'import', label: '导入时间' },
                                  { value: 'modified', label: '修改时间' },
                                  { value: 'visited', label: '最近访问' }
                                ].map(option => (
                                  <button
                                    key={option.value}
                                    onClick={() => handleUpdateSortMethod(option.value as SortMethod)}
                                    className={clsx(
                                      "px-3 py-1.5 text-xs rounded-md border transition-colors",
