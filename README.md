...
> **系统版本**: v1.0.0 | **项目性质**: 学科课程项目（南京理工大学 软件工程）
...
> 📌 **当前开发状态（v1.0.0 基础版本）**:
> - **已完成**：CHD 渲染引擎 · 双栏编辑器 · HTML 静态导出 · 文档全生命周期管理 · 操作引擎与撤销栈 · 测试与 CI 门禁
> - **开发中（协作进行）**：DeepSeek API 接入、AI 交互界面、编辑器内联编辑
>   当前仓库内 AI 部分为**占位骨架**：`/api/ai/*` 占位接口、`/ai-input` 实验页、首页实验入口、设置项「接入中」
...
| **AI 原生集成**（开发中） | 双模板 + 三级 Token 预算控制 | DeepSeek API + 本地代理收口（整包返回，Key 不出前端） |
...
- **AI 辅助生成（开发中）**: 规划支持将 `.txt` / `.md` / `.docx` 智能转换为 CHD 格式 Markdown（双模板：通用/学术）；当前为占位骨架。
...
详细测试数据来自项目自测（`npm run test:ci` / `npm run test:e2e`）与本地性能测试记录。
...
| AI（开发中） | DeepSeek API + 本地 API Route 代理 | 整包返回 + 重试机制 + Token 预算控制 |
...
详细设计见源码内注释与测试说明（`MdToHtml/tests/README.md`）。
...
> 上表为**完整版本**（AI 协作开发完成后）的实测数据；当前基础版本尚未包含 AI 相关测试套件。

覆盖重点：CHD 解析器（DFA 状态机边界，行覆盖 100%）、属性解析、操作引擎（13 种操作与逆操作）、HTML 导出（主路径 / 降级 / 转义）、文件与回收站服务、全文搜索。AI 代理相关用例（API Key 不出前端）随 AI 协作开发补入。
...
> 测试与 CI 门禁的组织方式见仓库内 [测试体系说明](MdToHtml/tests/README.md)。
...
| AI 集成 | 🔧 开发中（占位骨架已就位） | ❌ 无 | ❌ 需插件 | ⚠️ 社区插件 |
...
```bash
# 方式一：直接克隆（推荐）
# （备用镜像：https://gitee.com/njustzjh/md-to-html.git ）
git clone https://github.com/lihua6527-beep/MdToHtml.git
cd MdToHtml
npm install
start.bat
```
...
> 已超出 Git 托管平台的单文件大小限制（如 Gitee 为 **100 MB**），且其中绝大部分内容可由
...
│   └── tests/                     # 测试文件
├── scripts/                       # 仓库级脚本（归档记录 / git hooks 安装 / 端口初始化）
├── API接口手册.md                  # API 接口文档
├── README.md                      # 本文件
├── LICENSE                        # 木兰宽松许可证 (MulanPSL-2.0)
└── start.bat                      # 一键启动脚本
```

> `MdToHtml/CHD协议.md` 为 CHD 协议规范原文。
...
> 🔧 **开发中**：以下为协作开发中的功能清单，当前仓库内为占位骨架（占位接口 + 实验页）。
...
基础版本（CHD 渲染 / 双栏编辑器 / HTML 导出 / 文档管理 / 操作引擎 / 测试与 CI）已完成，
当前进入 **AI 能力协作开发**，按接口契约 → 引擎 → 服务端 → 配置 → 界面 的顺序推进：

- [ ] 接口契约与交接报文（请求/响应字段、错误码、模型枚举）
- [ ] Prompt 引擎外部化（`prompts.json` 热加载 + 兜底）与 API Key 热存储
- [ ] DeepSeek 服务端代理 + 前端服务层（超时控制 / 模型白名单 / 错误码映射 / 退避重试）
- [ ] 配置接口（保存 / 连通性检测）与生成路由回归测试
- [ ] AI 交互界面与编辑器内联编辑
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| v1.0.0 | 2026-09-21 | 基础版本：CHD 渲染、双栏编辑器、HTML 导出、文档管理、操作引擎、测试与 CI 门禁（AI 能力协作开发中） |
...
木兰宽松许可证 (MulanPSL-2.0) — 详见 [LICENSE](LICENSE)
