...
- `quote`: 引用/评价
- `code`: 纯代码展示
- `summary`: 综述/摘要（用于章节首部或尾部）
- `orange`: 强调/Hero样式（用于文档开头核心定位）
...
10. **禁止非标样式值**：`card-style` 必须使用协议定义的值（如 `normal`, `stat`, `summary` 等）。不支持的样式值（如 `info`, `custom` 等）将被忽略并回退到默认样式。
...
- **布局**：推荐使用单卡片全宽布局。
- **卡片样式**：使用 `orange` (强调/Hero) 或 `highlight` 样式。
- **属性配置**：`{card-style="orange", col-span=4, content-align="center", title-size="text-2xl"}`
...
- **样式搭配**：使用 `normal` 作为基础，配合 1-2 个 `highlight` 或 `stat` 卡片突出重点。

### 5.3 标准结尾 (Closing)
文档应以 **总结** 或 **下一步计划** 结束。
- **卡片样式**：使用 `summary` 样式。
- **属性配置**：`{card-style="summary"}`
- **目的**：收束全文，提供确定性的结论。

### 5.4 其他建议
1.  **字数控制**：单个卡片建议不超过 200 字。
2.  **形态搭配**：
    - **Wide Card** (`col-span=2`): 适合代码块、宽表格。
    - **Tall Card** (`row-span=2`): 适合竖向流程图。
