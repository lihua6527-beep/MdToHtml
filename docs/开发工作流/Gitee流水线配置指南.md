...
流水线由 `.workflow/` 下的文件定义（**文件名 = 流水线名**）。2026-09-14 **收敛后**的最终状态：

| 分支 | `.workflow/` 下的文件 | 说明 |
|------|----------------------|------|
| `master` | **`ci.yml`**（唯一） | **唯一生效的流水线**：`name: ci`，含「门禁」与「构建与E2E」两阶段 |
| `feat/exe-package-ready` / `test/ci-green` | **`ci.yml`** | 同一份内容（三分支同 SHA） |

> **收敛说明**：早先存在 3 份内容相同的流水线文件（`ci.yml` + 页面创建的两个时间戳文件），会导致**每次推送跑 3 遍**（重复消耗额度、日志与通知混乱）。已于 2026-09-14 删除两个时间戳文件，只保留 `ci.yml`。过程见 `docs/开发记录/历史记录/2026-09-14_开发记录.md` 第七批。

**关键结论**：不需要"导入文件"——Gitee 按 `.workflow/*.yml` 列出流水线；文件在哪个分支，就由该分支的推送 / 手动运行读取。
...
| Gitee 自动生成的文件 | `.workflow/流水线-202609142049.yml`（**文件名 = 流水线名**）→ 该文件已在 2026-09-14 收敛时删除，机制不变 |
...
2. 在流水线列表里点开 **`ci`**，点 **「运行」**（若要求选分支，选 `master`）
3. 看日志：按 Step 6 的表核对；**把日志或报错原文贴回来**，我据此校正

> **生效文件**：`.workflow/ci.yml`（`name: ci`）。若页面报 `displayName` 相关错误，把文件里的 `displayName` 改成 `ci`（与文件名一致）即可 —— 当前它是描述性文字。

### Step 4（已完成）：单条流水线，名为 `ci`

2026-09-14 已收敛为**一条**流水线：文件 `.workflow/ci.yml`，流水线名 `ci`。
若页面上还残留早先的 `流水线-2026091420xx` 条目，把它们删掉即可（那是已删除文件的旧记录）。
...
- 若页面提供「**代码源 → 触发设置**」：确认已勾选 **推送触发**，分支范围与 `.workflow/ci.yml` 的 `triggers` 一致
...
| 核对项 | 结果 |
|--------|------|
| `.workflow/` 下流水线文件 | ✅ **仅 `ci.yml` 一份**（2026-09-14 收敛：删除两个内容重复的时间戳文件）；三分支（`master` / `feat/exe-package-ready` / `test/ci-green`）同 SHA |
...
| 流水线文件结构 | js-yaml 解析 + 结构断言（version/name/displayName/triggers/stages/steps/commands/无 `inputs`/无 `stage:` 包裹） | ✅ `ci.yml` PASS（2 阶段 / 2 步骤，均 `build@nodejs`） |
