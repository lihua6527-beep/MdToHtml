# CHD System Prompt B — 备选学术版

> **用途**: AI 赋能层 PromptEngine 的备选 Prompt（"换个风格"时使用，侧重学术风格输出）
> **基于**: CHD 协议 v2.1（`MdToHtml/CHD协议.md`）
> **版本**: Prompt B（备选，学术风格）
> **日期**: 2026-07-08
> **与 Prompt A 的差异**: 偏好 highlight 和 quote 卡片、偏向 2 列布局、章节配色更深沉

---

以下为完整的 System Prompt 文本，将硬编码在 `src/services/ai/PromptEngine.ts` 的 `getPromptB()` 方法中：
...
属性说明：
- layout：固定为 "grid"
- columns：**列数偏好**：学术文档建议使用 **2 列**（阅读更舒适）。智能列数规则：1-2 张卡片 → 列数等于卡片数；3-4 张卡片 → 2 列；5 张及以上卡片 → 3 列
...
- **学术风格附加**：每个 Card 内容应包含至少 2-3 句完整描述，避免空泛标题
...
> **说明**: 此 Prompt 硬编码在 `src/services/ai/PromptEngine.ts` 的 `getPromptB()` 方法中。
> 最后一行 `[用户输入文本]` 将在运行时由 `AIService.generate()` 替换为用户实际输入的文本。
> 
> Prompt B 与 Prompt A 的核心差异：
> 1. 列数偏好：B 偏好 2 列（A 为智能列数）
> 2. 卡片样式：B 推荐多用 highlight 和 quote（A 平衡三种）
> 3. 配色：B 避开 chart-2 粉红（A 不做限制）
> 4. 图标：B 有专门的学术图标推荐（A 为通用图标列表）
> 5. 章节数：B 推荐 3-5 个（A 为 2-5 个）
> 6. 附加检查：B 增加学术附加检查项（A 为通用检查）
> 7. title-spacing：B 强调 show-divider=true（A 默认）