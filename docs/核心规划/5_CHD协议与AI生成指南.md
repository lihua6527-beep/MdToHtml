...
**核心要求**：
- 必须包含 `title`、`subtitle`、`tags` 三个必填字段
- 禁止重复键（如重复的 `status` 或 `training_sample`）
- 确保 YAML 格式正确，使用正确的缩进和引号
...
subtitle: "YAML 格式错误"
tags: ["错误", "示例"]
status: incomplete  # 重复键
raining_sample: false  # 拼写错误
---
```
...
**核心要求**：
- 必须为每个 Section 指定 `layout="grid"` 属性（其他布局已锁定）
- 可选指定 `columns` 属性，遵循智能列数规则
- 可选指定 `section-color` 属性，使用莫兰迪主题色
...
**错误示例**：
```markdown
## 错误示例 {layout="single"}  # 禁止使用 single 布局
...
**正确示例**：
```markdown
## 正确示例 {layout="grid" section-color="chart-1"}

## 另一个正确示例 {layout="grid" columns=3}
```
...
**核心要求**：
- 必须为每个卡片指定 `card-style` 属性
- 仅允许使用 `normal`、`highlight`、`quote` 三种样式
- 严禁使用 `code`、`stat`、`warning`、`summary` 等样式
- 确保 `card-style` 属性值使用双引号
- 禁止使用 `col-span` 属性（已弃用）
...
**错误示例**：
```markdown
### 错误示例 {card-style=code}  # 禁止使用 code 样式，且缺少双引号
这里是错误的卡片内容...

### 另一个错误示例 {card-style="normal" col-span=2}  # 禁止使用 col-span 属性
这里是另一个错误的卡片内容...
```

**正确示例**：
```markdown
### 正确示例 {card-style="normal"}
这里是正确的卡片内容...

### 另一个正确示例 {card-style="highlight"}
这里是另一个正确的卡片内容...

### 引用示例 {card-style="quote"}
> 这里是引用内容...
```
