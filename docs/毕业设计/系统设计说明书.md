...
- **代理调用（整包返回）**：浏览器经本地 `/api/ai/generate` API Route 代理调用 DeepSeek，API Key 只在服务端读取、不出前端；当前为单请求整包返回 CHD Markdown（SSE 流式作为可扩展方向，现网未启用）
- **错误重试**：网络错误时重试 3 次，间隔递增（1s → 2s → 4s），另有 120s 超时中止

#### 核心接口

```typescript
class AIService {
  generate(params: AIGenerationParams): Promise<AIResult>;  // 单请求整包返回（见 4.4 设计要点）
